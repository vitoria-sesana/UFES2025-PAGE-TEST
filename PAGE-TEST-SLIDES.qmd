...
::: info-direita
Universidade Federal do Espírito Santo<br> Centro de Ciências Exatas<br> Curso de Estatística Aplicada
:::
...
A história do **Teste de Page** tem origem no contexto dos testes não paramétricos desenvolvidos no século XX, e está associada ao psicológo e estatístico americano [Ellis Batten Page]{.underline}, que propôs o teste em 1963[^1].

[^1]: Page, E. B. (1963). "Ordered hypotheses for multiple treatments: a significance test for linear ranks." Journal of the American Statistical Association, 58(301), 216–230.
...
[^2]: Código doi: 10.1080/01621459.1963.10500843
...
6.  Comparar com valor crítico ou usar aproximação normal para obter p-valor.
...
-   $R_{j}$ = soma dos ranks do tratamento.[^3]

[^3]: $R_{j} =  \sum{}_{i=1}^{b}X_{ij}$ com $i = {1,\dots,b}$ e $j = {1,\dots,k}$
...
ggplot(dados_plot, aes(x = frequencia_ru, y = n)) +
  geom_bar(stat = "identity", fill = "#364261", width = 0.75) +                
  geom_text(aes(label = label), colour = "#364261",vjust = -0.5) +  
  labs(title = "",
       x = "",
       y = "Frequência das respostas") +
  scale_y_continuous(
    limits = c(0, max(dados_plot$n) * 1.1),
    breaks = seq(0, max(dados_plot$n), by = 2)
  ) +
  theme_minimal() + 
  theme( 
    panel.grid.major.x = element_blank(),
    panel.grid.minor.y = element_line(colour = "#C4C4C4"),
    panel.grid.major.y = element_line(colour = "#C4C4C4")
  ) 
```
...
# Selecionar colunas dos alimentos
dados <- base %>% 
  filter( frequencia_ru %in% c("Frequentemente")) %>% 
  select(
    carne_moida,
    cozido_misto,
    frango_grelhado,
